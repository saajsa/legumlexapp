package com.legumlex.clientapp.ui.theme

import androidx.compose.ui.graphics.Color

// LegumLex Brand Colors
val LegumLexPrimary = Color(0xFF1565C0) // Professional blue
val LegumLexPrimaryVariant = Color(0xFF0D47A1) // Darker blue
val LegumLexSecondary = Color(0xFF37474F) // Professional gray
val LegumLexSecondaryVariant = Color(0xFF263238) // Darker gray

// Additional colors for legal industry
val LegumLexGold = Color(0xFFFFB300) // For premium/important items
val LegumLexSuccess = Color(0xFF2E7D32) // For success states
val LegumLexWarning = Color(0xFFEF6C00) // For warnings
val LegumLexError = Color(0xFFD32F2F) // For errors

// Background colors
val LegumLexSurface = Color(0xFFFAFAFA)
val LegumLexBackground = Color(0xFFFFFFFF)

// Text colors
val LegumLexOnPrimary = Color(0xFFFFFFFF)
val LegumLexOnSecondary = Color(0xFFFFFFFF)
val LegumLexOnSurface = Color(0xFF212121)
val LegumLexOnBackground = Color(0xFF212121)

// Dark theme colors
val LegumLexPrimaryDark = Color(0xFF1976D2)
val LegumLexPrimaryVariantDark = Color(0xFF1565C0)
val LegumLexSecondaryDark = Color(0xFF546E7A)
val LegumLexSecondaryVariantDark = Color(0xFF37474F)

val LegumLexSurfaceDark = Color(0xFF121212)
val LegumLexBackgroundDark = Color(0xFF121212)
val LegumLexOnSurfaceDark = Color(0xFFE0E0E0)
val LegumLexOnBackgroundDark = Color(0xFFE0E0E0)

// Status colors
val StatusOpen = Color(0xFF2196F3) // Blue
val StatusInProgress = Color(0xFFFF9800) // Orange
val StatusCompleted = Color(0xFF4CAF50) // Green
val StatusClosed = Color(0xFF9E9E9E) // Gray
val StatusOverdue = Color(0xFFF44336) // Red
val StatusPaid = Color(0xFF4CAF50) // Green
val StatusUnpaid = Color(0xFFFF5722) // Deep Orange

// Priority colors
val PriorityLow = Color(0xFF81C784) // Light Green
val PriorityMedium = Color(0xFFFFB74D) // Light Orange
val PriorityHigh = Color(0xFFE57373) // Light Red
val PriorityUrgent = Color(0xFFAD1457) // Dark Pink